...
def search(start):
    ...
    while open != []:
        ...
        curr = open[0] # Copy over the first item in open
        open = open[1:] # Slice from second element
        closed.append(curr)  # store copy on closed;

        #show_puzz8(curr.state)

        #print "%d. From open... (%d) %s" % (k,curr.no,curr.state)
        #print("test", curr.state)

        if succesor_fct(curr.state):
            #return True  
            print("Found a path!")
